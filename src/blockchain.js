...
class Blockchain {
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            const request_time = Time.now();
            const message = `${address}:${request_time}:starRegistry`;
            resolve(message);
        });
    }
     ...
     */
     getStarsByWalletAddress (address) {
        ...
        self.chain.forEach(async(block) => {
            ...
            if(blockDecoded.address){    
                if(blockDecoded.address === address){
                    stars.push(blockDecoded);
                }
                else {
                    reject(new Error('Wrong Address or address not found.'));
                }
            }
            resolve(stars);
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
           ...
           self.chain.forEach(block => {
               ...
               if (block.height >0) {
                   ...
                   if (blockHash != previousBlockHash) {
                       errorLog.push(' Block Height ${block.height} - does not match with previous hash.')
                   }
               }
           ...
           Promise.all(promises).then((results) => {
                ...
                if (!valid) {
                   error.push(' Block Height: ${self.chain[chainIndex].height} - was tampered!');
